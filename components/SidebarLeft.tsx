...
const SidebarLeft: React.FC = () => {
  ...
  return (
    <aside className="w-64 h-full glass-panel border-r border-white/10 flex flex-col z-20 hidden md:flex">
         ...
         <Panel title="History">
           <div className="space-y-1 max-h-64 overflow-y-auto">
             {state.history.map((_, idx) => (
               <div
                 key={idx}
                 className={`p-2 text-[10px] rounded cursor-pointer ${
                   idx === state.historyIndex ? 'bg-blue-600/20 text-blue-400' : 'text-gray-500 hover:bg-white/5'
                 }`}
                 onClick={() => {
