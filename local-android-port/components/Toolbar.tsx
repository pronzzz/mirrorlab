...
const Toolbar: React.FC = () => {
  ...
  const handleExport = async () => {
    ...
    if (canvas) {
      if (Capacitor.isNativePlatform()) {
        try {
          const base64 = canvas.toDataURL(exportFormat, quality).split(',')[1];
          const fileName = `${exportName || 'edited-image'}.${exportFormat.split('/')[1]}`;
  ...
  return (
    <header className="h-16 flex items-center justify-between px-4 md:px-6 z-30 relative bg-background border-b border-white/5 shadow-lg">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          ...
          {showExportOptions && (
            <div className="absolute right-0 top-full mt-2 md:mt-4 w-72 md:w-80 bg-surface neumorphic border border-white/5 p-5 z-50 animate-scale-in origin-top-right shadow-2xl mr-2 md:mr-0">
